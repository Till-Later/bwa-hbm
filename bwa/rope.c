...
typedef struct
{  // memory pool for fast and compact memory allocation (no free)
    int size, i, n_elems;
    int64_t top, max;
    uint8_t** mem;
} mempool_t;
...
static inline rpnode_t* split_node(
    rope_t* rope,
    rpnode_t* u,
    rpnode_t*
        v) {  // split $v's child. $u is the first node in the bucket. $v and $u are in the same bucket. IMPORTANT: there is always enough room in $u
    int j, i = v - u;
    rpnode_t* w;   // $w is the sibling of $v
    if (u == 0) {  // only happens at the root; add a new root
        u = v = mp_alloc(rope->node);
        v->n = 1;
        v->p = rope->root;  // the new root has the old root as the only child
    ...
    if (u->is_bottom) {  // we are at the bottom level; $v->p is a string instead of a node
        ...
        q->n = rope->max_nodes
            >> 1;  // NB: this line must below memcpy() as $q->n and $q->is_bottom are modified by memcpy()
...
int64_t rope_insert_run(
    rope_t* rope,
    int64_t x,
    int a,
    int64_t rl,
    rpcache_t* cache) {  // insert $a after $x symbols in $rope and the returns rank(a, x)
    rpnode_t *
        u = 0,
       *v = 0,
       *p =
           rope->root;  // $v is the parent of $p; $u and $v are at the same level and $u is the first node in the bucket
    ...
    do {                                // top-down update. Searching and node splitting are done together in one pass.
        if (p->n == rope->max_nodes) {  // node is full; split
            v = split_node(
                rope, u, v);   // $v points to the parent of $p; when a new root is added, $v points to the root
        ...
        if (v && x - y > v->l >> 1) {  // then search backwardly for the right node to descend
            ...
            for (; y + p->l < x; ++p)
                y += p->l, z += p->c[a];  // then search forwardly
        assert(p - u < u->n);
        if (v)
            v->c[a] += rl,
                v->l += rl;  // we should not change p->c[a] because this may cause troubles when p's child is split
        v = p;
        p = p->p;  // descend
    } while (!u->is_bottom);
    rope->c[a] += rl;  // $rope->c should be updated after the loop as adding a new root needs the old $rope->c counts
    if (cache) {
        if (cache->p != (uint8_t*)p)
    ...
    z += cnt[a];
    v->c[a] += rl;
    v->l += rl;  // this should be after rle_insert(); otherwise rle_insert() won't work
...
const uint8_t* rope_itr_next_block(rpitr_t* i) {
    ...
    while (i->d >= 0 && ++i->ia[i->d] == i->pa[i->d]->n)
        i->ia[i->d--] = 0;  // backtracking
    if (i->d >= 0)
