...
bwa_seqio_t* bwa_bam_open(const char* fn, int which) {
    ...
    bs->fp = bam_open(fn, "r");
    if (0 == bs->fp)
        err_fatal_simple("Couldn't open bam file");
...
void bwa_seq_close(bwa_seqio_t* bs) {
    if (bs == 0)
        return;
    if (bs->is_bam) {
        if (0 != bam_close(bs->fp))
            err_fatal_simple("Error closing bam file");
    } else {
...
static bwa_seq_t* bwa_read_bam(bwa_seqio_t* bs, int n_needed, int* n, int is_comp, int trim_qual) {
    ...
    while ((res = bam_read1(bs->fp, b)) >= 0) {
        uint8_t *s, *q;
        int go = 0;
        if ((bs->which & 1) && (b->core.flag & BAM_FREAD1))
            go = 1;
        if ((bs->which & 2) && (b->core.flag & BAM_FREAD2))
            go = 1;
        if ((bs->which & 4) && !(b->core.flag & BAM_FREAD1) && !(b->core.flag & BAM_FREAD2))
            go = 1;
        if (go == 0)
            continue;
        l = b->core.l_qseq;
    ...
    if (res < 0 && res != -1)
        err_fatal_simple("Error reading bam file");
    *n = n_seqs;
...
bwa_seq_t* bwa_read_seq(bwa_seqio_t* bs, int n_needed, int* n, int mode, int trim_qual) {
    ...
    while ((l = kseq_read(seq)) >= 0) {
        ...
        if (l_bc) {  // then trim barcode
            for (i = 0; i < l_bc; ++i)
                p->bc[i] = (seq->qual.l && seq->qual.s[i] - 33 < BARCODE_LOW_QUAL) ? tolower(seq->seq.s[i])
                                                                                   : toupper(seq->seq.s[i]);
            p->bc[i] = 0;
            for (; i < seq->seq.l; ++i)
