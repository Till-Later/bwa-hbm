...
static int infer_isize(int n_seqs, bwa_seq_t* seqs[2], isize_info_t* ii, double ap_prior, int64_t L) {
    ...
    for (y = 1.0; y < 10.0; y += 0.01)
        if (.5 * erfc(y / M_SQRT2) < ap_prior / L * (y * ii->std + ii->avg))
            break;
    ...
    if (ii->ap_prior < ap_prior)
        ii->ap_prior = ap_prior;
    free(isizes);
    ...
    fprintf(
        stderr,
        "[infer_isize] low and high boundaries: %ld and %ld for estimating avg and std\n",
        (long)ii->low,
        (long)ii->high);
    fprintf(stderr, "[infer_isize] inferred external isize from %d pairs: %.3lf +/- %.3lf\n", n, ii->avg, ii->std);
    fprintf(
        stderr, "[infer_isize] skewness: %.3lf; kurtosis: %.3lf; ap_prior: %.2e\n", skewness, kurtosis, ii->ap_prior);
...
static int pairing(bwa_seq_t* p[2], pe_data_t* d, const pe_opt_t* opt, int s_mm, const isize_info_t* ii) {
    ...
    if (max_len < p[1]->full_len)
        max_len = p[1]->full_len;
    if (low_bound < max_len)
        low_bound = max_len;

        // here v>=u. When ii is set, we check insert size with ii; otherwise with opt->max_isize
#define __pairing_aux(u, v) \
    do { \
        ...
        if ((u).x != (uint64_t)-1 && (v).x > (u).x && l >= max_len \
            ...
            if (s >> 32 == o_score >> 32) \
                ++o_n; \
            else if (s >> 32 < o_score >> 32) { \
                subo_n += o_n; \
                o_n = 1; \
            } else \
                ++subo_n; \
            if (s < o_score) \
                subo_score = o_score, o_score = s, o_pos[(u).y & 1] = (u), o_pos[(v).y & 1] = (v); \
            else if (s < subo_score) \
                subo_score = s; \
        } \
    } while (0)

#define __pairing_aux2(q, w) \
    do { \
    ...
    for (j = 0; j < 2; ++j)
        last_pos[j][0].x = last_pos[j][0].y = last_pos[j][1].x = last_pos[j][1].y = (uint64_t)-1;
    ...
    if (o_score != (uint64_t)-1) {
        ...
        //fprintf(stderr, "%d, %d\n", o_n, subo_n);
        if (o_n == 1) {
            if (subo_score == (uint64_t)-1)
                mapQ_p = 29;  // no sub-optimal pair
            else if ((subo_score >> 32) - (o_score >> 32) > s_mm * 10)
        ...
        if ((p[0]->pos == o_pos[0].x && p[0]->strand == (o_pos[0].y >> 1 & 1))
            && (p[1]->pos == o_pos[1].x && p[1]->strand == (o_pos[1].y >> 1 & 1))) {  // both ends not moved
...
int bwa_cal_pac_pos_pe(
    ...
    for (i = 0; i != n_seqs; ++i) {
        ...
        for (j = 0; j < 2; ++j) {
            ...
            if (p[j]->type == BWA_TYPE_UNIQUE || p[j]->type == BWA_TYPE_REPEAT) {
                ...
                p[j]->strand = strand;
                if (p[j]->pos == (bwtint_t)-1)
                    p[j]->type = BWA_TYPE_NO_MATCH;
            }
        }
    }
    ...
    for (i = 0; i != n_seqs; ++i) {
        ...
        if ((p[0]->type == BWA_TYPE_UNIQUE || p[0]->type == BWA_TYPE_REPEAT)
            && (p[1]->type == BWA_TYPE_UNIQUE || p[1]->type == BWA_TYPE_REPEAT)) {  // only when both ends mapped
            pair64_t x;
            int j, k;
            long long n_occ[2];
            for (j = 0; j < 2; ++j) {
...
bwa_cigar_t* bwa_sw_core(
    ...
    if (reglen < SW_MIN_MATCH_LEN || (int64_t)l_pac - *beg < len)
        return 0;
    for (k = 0, x = 0; k < len; ++k)
        if (seq[k] >= 4)
            ++x;
    if ((float)x / len >= 0.25 || len - x < SW_MIN_MATCH_LEN)
        return 0;
    ...
    if (r.score < SW_MIN_MATCH_LEN || r.score2 == r.score
        || gscore != r.score) {  // poor hit or tandem hits or weird alignment
        free(cigar);
        free(ref_seq);
        *n_cigar = 0;
        return 0;
        ...
        for (k = 0; k < *n_cigar; ++k) {
            ...
            if (__cigar_op(c) == FROM_M) {
                for (l = 0; l < (__cigar_len(c)); ++l)
                    if (ref_seq[x + l] < 4 && seq[y + l] < 4 && ref_seq[x + l] != seq[y + l])
                        ++n_mm;
                x += __cigar_len(c), y += __cigar_len(c);
            } else if (__cigar_op(c) == FROM_D) {
...
ubyte_t* bwa_paired_sw(
    const bntseq_t* bns,
    const ubyte_t* _pacseq,
    int n_seqs,
    bwa_seq_t* seqs[2],
    const pe_opt_t* popt,
    ...
    for (i = 0; i != n_seqs; ++i) {
        ...
        p[0] = seqs[0] + i;
        p[1] = seqs[1] + i;
        if ((p[0]->mapQ >= SW_MIN_MAPQ || p[1]->mapQ >= SW_MIN_MAPQ)
            && (p[0]->extra_flag & SAM_FPP) == 0) {  // unpaired and one read has high mapQ
...
#define __set_rght_coor(_a, _b, _pref, _pmate) \
    do { \
        (_a) = (int64_t)_pref->pos + ii->avg - 3 * ii->std - _pmate->len * 1.5; \
        (_b) = (_a) + 6 * ii->std + 2 * _pmate->len; \
        if ((_a) < (int64_t)_pref->pos + _pref->len) \
            (_a) = _pref->pos + _pref->len; \
        if ((_b) > bns->l_pac) \
            (_b) = bns->l_pac; \
    } while (0)

#define __set_left_coor(_a, _b, _pref, _pmate) \
    do { \
...
#define __set_fixed(_pref, _pmate, _beg, _cnt) \
    do { \
        ...
        _pmate->n_mm = _cnt >> 16; \
        _pmate->n_gapo = _cnt >> 8 & 0xff; \
        _pmate->n_gape = _cnt & 0xff; \
        ...
        _pref->extra_flag |= SAM_FPP; \
            ...
            for (k = 0; k < 2; ++k) {  // p[1-k] is the reference read and p[k] is the read considered to be modified
                ubyte_t* seq;
                if (p[1 - k]->type == BWA_TYPE_NO_MATCH)
                    continue;  // if p[1-k] is unmapped, skip
                ...
                cigar[k] =
                    bwa_sw_core(bns->l_pac, pacseq, p[k]->len, seq, &beg[k], end[k] - beg[k], &n_cigar[k], &cnt[k]);
                if (cigar[k] && p[k]->type != BWA_TYPE_NO_MATCH) {  // re-evaluate cigar[k]
                    int s_old, clip = 0, s_new;
                    if (__cigar_op(cigar[k][0]) == 3)
                        clip += __cigar_len(cigar[k][0]);
                    if (__cigar_op(cigar[k][n_cigar[k] - 1]) == 3)
                        clip += __cigar_len(cigar[k][n_cigar[k] - 1]);
                    s_old = (int)((p[k]->n_mm * 9 + p[k]->n_gapo * 13 + p[k]->n_gape * 2) / 3. * 8. + .499);
                    s_new =
                        (int)(((cnt[k] >> 16) * 9 + (cnt[k] >> 8 & 0xff) * 13 + (cnt[k] & 0xff) * 2 + clip * 3) / 3. * 8. + .499);
                    s_old += -4.343 * log(ii->ap_prior / bns->l_pac);
                    s_new +=
                        (int)(-4.343 * log(.5 * erfc(M_SQRT1_2 * 1.5) + .499));  // assume the mapped isize is 1.5\sigma
            ...
            if (k >= 0 && p[k]->pos != beg[k]) {
                ...
                {  // recalculate mapping quality
                    ...
                    if (p[k]->mapQ > mq_adjust[k])
                        p[k]->mapQ = mq_adjust[k];
                    if (p[k]->seQ > mq_adjust[k])
                        p[k]->seQ = mq_adjust[k];
                }
                // update CIGAR
                free(p[k]->cigar);
    ...
    fprintf(
        ...
        (long long)n_mapped[1],
        (long long)n_tot[1],
        SW_MIN_MAPQ);
    fprintf(
        stderr,
        "[bwa_paired_sw] %lld out of %lld Q%d discordant pairs are fixed.\n",
        (long long)n_mapped[0],
        (long long)n_tot[0],
        SW_MIN_MAPQ);
    return pacseq;
}

void bwa_sai2sam_pe_core(
    const char* prefix,
    char* const fn_sa[2],
    char* const fn_fa[2],
    pe_opt_t* popt,
...
int bwa_sai2sam_pe(int argc, char* argv[]) {
    ...
    while ((c = getopt(argc, argv, "a:o:sPn:N:c:f:Ar:")) >= 0) {
        ...
        case 'r':
            ...
            break;
        case 'a':
            popt->max_isize = atoi(optarg);
            break;
        case 'o':
            popt->max_occ = atoi(optarg);
            break;
        case 's':
            popt->is_sw = 0;
            break;
        case 'P':
            ...
            break;
        case 'n':
            popt->n_multi = atoi(optarg);
            break;
        case 'N':
            popt->N_multi = atoi(optarg);
            break;
        case 'c':
            popt->ap_prior = atof(optarg);
            break;
        case 'f':
