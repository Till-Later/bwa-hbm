...
for {set i 0} {$i < $hbm_axi_if_num} {incr i} {
  ...
  connect_bd_net [get_bd_ports S_AXI_p0_HBM_ACLK] [get_bd_pins axi4_to_axi3_$i/aclk]
  ...
  connect_bd_net [get_bd_ports S_AXI_p0_HBM_ACLK] [get_bd_pins axi_register_slice_$i\/aclk]
  ...
  if { $i < 10} {
    ...
    connect_bd_net [get_bd_ports S_AXI_p0_HBM_ACLK] [get_bd_pins hbm/AXI_0$i\_ACLK]
    ...
    connect_bd_net [get_bd_ports S_AXI_p0_HBM_ACLK] [get_bd_pins hbm/AXI_$i\_ACLK]
    connect_bd_intf_net [get_bd_intf_pins axi_register_slice_$i\/M_AXI] [get_bd_intf_pins hbm/SAXI_$i]
  }
}
#--------------------- end loop ------------------

# Drive all clock signals through clock port of S_AXI_p0
set clk_port [get_bd_ports S_AXI_p0_HBM_ACLK]

#-- AXI Crossbars
for {set crossbar_index 0} {$crossbar_index < 4} {incr crossbar_index} {
  create_bd_cell -type ip -vlnv xilinx.com:ip:axi_crossbar:2.1 axi_crossbar_$crossbar_index
  ...
  set_property -dict {
    ...
    CONFIG.M01_WRITE_ISSUING {32}
    CONFIG.M02_WRITE_ISSUING {32}
    CONFIG.M03_WRITE_ISSUING {32}
    CONFIG.M00_READ_ISSUING {32}
    CONFIG.M01_READ_ISSUING {32}
    CONFIG.M02_READ_ISSUING {32}
    CONFIG.M03_READ_ISSUING {32}
    ...
    CONFIG.M03_A00_BASE_ADDR {0x00000000c0000000}
    CONFIG.M00_A00_ADDR_WIDTH {30}
    CONFIG.M01_A00_ADDR_WIDTH {30}
    CONFIG.M02_A00_ADDR_WIDTH {30}
    CONFIG.M03_A00_ADDR_WIDTH {30}
  } [get_bd_cells axi_crossbar_$crossbar_index]

  for {set si_index 0} {$si_index < 4} {incr si_index} {
    set global_index [expr 4 * $crossbar_index + $si_index]

    set register_slice [create_bd_cell -type ip -vlnv {xilinx.com:ip:axi_register_slice:*} axi_outer_reg_$global_index ]
    set_property -dict {     \
      CONFIG.ADDR_WIDTH {34}              \
      CONFIG.DATA_WIDTH {256}             \
      CONFIG.ID_WIDTH {6}                 \
    ...
    connect_bd_net [get_bd_ports CRESETN] [get_bd_pins axi_outer_reg_$global_index\/aresetn]
    connect_bd_net [get_bd_ports S_AXI_p0_HBM_ACLK] [get_bd_pins axi_outer_reg_$global_index\/aclk]

    connect_bd_intf_net [get_bd_intf_ports S_AXI_p$global_index\_HBM] [get_bd_intf_pins axi_outer_reg_$global_index\/S_AXI]
    connect_bd_intf_net [get_bd_intf_pins axi_outer_reg_$global_index\/M_AXI] [get_bd_intf_pins axi_crossbar_$crossbar_index\/S[format "%02d" $si_index]\_AXI]
  }

  if { $crossbar_index < 4} {
    connect_bd_intf_net [get_bd_intf_pins axi_crossbar_$crossbar_index/M00_AXI] [get_bd_intf_pins axi4_to_axi3_[expr 0 + $crossbar_index]/S_AXI]
    connect_bd_intf_net [get_bd_intf_pins axi_crossbar_$crossbar_index/M01_AXI] [get_bd_intf_pins axi4_to_axi3_[expr 4 + $crossbar_index]/S_AXI]
    connect_bd_intf_net [get_bd_intf_pins axi_crossbar_$crossbar_index/M02_AXI] [get_bd_intf_pins axi4_to_axi3_[expr 8 + $crossbar_index]/S_AXI]
    connect_bd_intf_net [get_bd_intf_pins axi_crossbar_$crossbar_index/M03_AXI] [get_bd_intf_pins axi4_to_axi3_[expr 12 + $crossbar_index]/S_AXI]
  } else {
    connect_bd_intf_net [get_bd_intf_pins axi_crossbar_$crossbar_index/M00_AXI] [get_bd_intf_pins axi4_to_axi3_[expr 12 + $crossbar_index]/S_AXI]
    connect_bd_intf_net [get_bd_intf_pins axi_crossbar_$crossbar_index/M01_AXI] [get_bd_intf_pins axi4_to_axi3_[expr 16 + $crossbar_index]/S_AXI]
    connect_bd_intf_net [get_bd_intf_pins axi_crossbar_$crossbar_index/M02_AXI] [get_bd_intf_pins axi4_to_axi3_[expr 20 + $crossbar_index]/S_AXI]
    connect_bd_intf_net [get_bd_intf_pins axi_crossbar_$crossbar_index/M03_AXI] [get_bd_intf_pins axi4_to_axi3_[expr 24 + $crossbar_index]/S_AXI]
  }

  connect_bd_net [get_bd_pins CRESETN] [get_bd_pins axi_crossbar_$crossbar_index/aresetn]
  connect_bd_net $clk_port [get_bd_pins axi_crossbar_$crossbar_index/aclk]
}

for {set i 16} {$i < $hbm_axi_if_num} {incr i} {
...
connect_bd_net [get_bd_pins hbm/HBM_REF_CLK_0] $clk_port
connect_bd_net $clk_port [get_bd_pins refclk_bufg_apb_clk/BUFGCE_I]
if { $hbm_axi_if_num > 16 } {
  connect_bd_net [get_bd_pins hbm/HBM_REF_CLK_1] $clk_port
}

#assign_bd_address >> $log_file
for {set offset_factor 0} {$offset_factor < 16} {incr offset_factor} {
  for {set slave_segment 0} {$slave_segment < 4} {incr slave_segment} {
    assign_bd_address -offset 0x[format "%01x" $offset_factor]0000000 -range 256M [get_bd_addr_segs hbm/SAXI_[format "%02d" [expr 4 * ($offset_factor / 4) + $slave_segment]]\/HBM_MEM[format "%02d" $offset_factor]] >> $log_file
    assign_bd_address -offset 0x1[format "%01x" $offset_factor]0000000 -range 256M [get_bd_addr_segs hbm/SAXI_[format "%02d" [expr 16 + 4 * ($offset_factor / 4) + $slave_segment]]\/HBM_MEM[format "%02d" $offset_factor]] >> $log_file
  }
}
