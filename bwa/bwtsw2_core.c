...
inline static bsw2entry_p mp_alloc(mempool_t* mp) {
    ++mp->cnt;
    if (kv_size(mp->pool) == 0)
        return (bsw2entry_t*)calloc(1, sizeof(bsw2entry_t));
    else
        return kv_pop(mp->pool);
}
...
static void cut_tail(bsw2entry_t* u, int T, bsw2entry_t* aux) {
    ...
    a = (int*)aux->array;
    for (i = n = 0; i != u->n; ++i)
        if (u->array[i].ql && u->array[i].G > 0) a[n++] = -u->array[i].G;
    if (n <= T) return;
...
static inline void remove_duplicate(bsw2entry_t* u, khash_t(qintv) * hash) {
    ...
    for (i = 0; i != u->n; ++i) {
        ...
        j = -1;
        if (ret == 0) {
            if ((uint32_t)kh_value(hash, k) >= p->G)
                j = i;
            else {
        ...
        for (j = 0; j < i; ++j) {
            ...
            if (p->l == 0 && q->l == 0) {
                ...
                if ((float)qol / (p->end - p->beg) > MASK_LEVEL || (float)qol / (q->end - q->beg) > MASK_LEVEL) {
                    ...
                    if ((double)tol / p->len > MASK_LEVEL || (double)tol / q->len > MASK_LEVEL) compatible = 0;
                }
            }
            if (!compatible) {
                p->G = 0;
