...
bam_header_t* bam_header_read(bamFile fp) {
    ...
    if (bam_read(fp, &header->l_text, 4) != 4)
        goto fail;
    if (bam_is_be)
        bam_swap_endian_4p(&header->l_text);
    header->text = (char*)calloc(header->l_text + 1, 1);
    if (bam_read(fp, header->text, header->l_text) != header->l_text)
        goto fail;
    if (bam_read(fp, &header->n_targets, 4) != 4)
        goto fail;
    if (bam_is_be)
    ...
    for (i = 0; i != header->n_targets; ++i) {
        ...
        if (bam_read(fp, &header->target_len[i], 4) != 4)
            goto fail;
        if (bam_is_be)
...
int bam_read1(bamFile fp, bam1_t* b) {
    ...
    if (bam_read(fp, x, sizeof(bam1_core_t)) != sizeof(bam1_core_t))
        return -3;
    if (bam_is_be) {
...
gzFile bamlite_gzopen(const char* fn, const char* mode) {
    ...
    if (strcmp(fn, "-") == 0) {
        ...
        if (!fp) {
            fprintf(stderr, "Couldn't open %s : %s", (strstr(mode, "r")) ? "stdin" : "stdout", strerror(errno));
        }
        return fp;
    }
