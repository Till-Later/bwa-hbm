...
if { [info exists ::env(SYNTH_DESIGN_DIRECTIVE)] == 1 } {
  set directive $env(SYNTH_DESIGN_DIRECTIVE);
}
...
if { [info exists ::env(SYNTH_DESIGN_RESOURCE_SHARING)] == 1 } {
  append command " -resource_sharing  $env(SYNTH_DESIGN_RESOURCE_SHARING)"
} else {
  append command " -resource_sharing  [get_property STEPS.SYNTH_DESIGN.ARGS.RESOURCE_SHARING  [get_runs synth_1]]"
}
if { [info exists ::env(SYNTH_DESIGN_SHREG_MIN_SIZE)] == 1 } {
  append command " -shreg_min_size  $env(SYNTH_DESIGN_SHREG_MIN_SIZE)"
} else {
...
if { [info exists ::env(SYNTH_DESIGN_NO_LC)] == 1 } {
  if { $env(SYNTH_DESIGN_NO_LC) == 1 } {
    append command " -no_lc"
  }
} elseif { [get_property STEPS.SYNTH_DESIGN.ARGS.NO_LC [get_runs synth_1]] == 1 } {
  append command " -no_lc"
}

if { [info exists ::env(SYNTH_DESIGN_RETIMING)] == 1 } {
  append command " -retiming  $env(SYNTH_DESIGN_RETIMING)"
}
