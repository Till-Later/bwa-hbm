...
unsigned char nt_comp_table[256] = {
    ...
    'V', 'G', 'H', 'N', 'N', 'C', 'D', 'N', 'N', 'M', 'N', 'K', 'N', 'N', 'N', 'N', 'Y', 'S', 'A', 'N', 'B', 'W',
    'X', 'R', 'N', 'N', 'N', 'N', 'N', 'N', 'n', 't', 'v', 'g', 'h', 'n', 'n', 'c', 'd', 'n', 'n', 'm', 'n', 'k',
    'n', 'n', 'n', 'n', 'y', 's', 'a', 'n', 'b', 'w', 'x', 'r', 'n', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N',
    'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N',
    'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N',
    'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N',
    'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N',
...
void bsw2_extend_left(
    ...
    for (i = 0; i < b->n; ++i) {
        ...
        lt = j;
        score = ksw_extend(
            p->beg, &query[lq - p->beg], lt, target, 5, mat, opt->q, opt->r, opt->bw, 0, -1, p->G, &qle, &tle, 0, 0, 0);
...
void bsw2_extend_rght(
    ...
    for (i = 0; i < b->n; ++i) {
        ...
        score =
            ksw_extend(
                lq - p->beg, &query[p->beg], lt, target, 5, mat, opt->q, opt->r, opt->bw, 0, -1, 1, &qle, &tle, 0, 0, 0)
            - 1;
...
void bsw2_debug_hits(const bwtsw2_t* b) {
    ...
    for (i = 0; i < b->n; ++i) {
        ...
        if (p->G > 0)
            printf(
                "G=%d, G2=%d, len=%d, [%d,%d), k=%lu, l=%lu, #seeds=%d, is_rev=%d\n",
                p->G,
                p->G2,
                p->len,
                p->beg,
...
static bwtsw2_t* bsw2_aln1_core(
    const bsw2opt_t* opt,
    const bntseq_t* bns,
    uint8_t* pac,
    const bwt_t* target,
    ...
    b[0] = bb[0][1];
    b[1] = bb[1][1];               // bb[*][1] are "narrow SA hits"
...
static void write_aux(
    ...
    for (i = 0; i < b->n; ++i) {
        ...
        if (p->l == 0) {  // unique hit
            ...
            q->qual = 0, q->n_cigar = 0, q->chr = q->pos = -1, q->nn = 0;
    }
}
...
static void update_mate_aux(bwtsw2_t* b, const bwtsw2_t* m) {
    ...
    for (i = 0; i < b->n; ++i) {
        ...
        q->flag |= 1;  // paired
        if (m->n == 0)
            q->flag |= 8;  // mate unmapped
        if (m->n == 1) {
            q->mchr = m->aux[0].chr;
            q->mpos = m->aux[0].pos;
            if (m->aux[0].flag & 0x10)
                q->flag |= 0x20;      // mate reverse strand
            if (q->chr == q->mchr) {  // set insert size
                if (q->mpos + m->hits[0].len > q->pos)
                    q->isize = q->mpos + m->hits[0].len - q->pos;
                else
                    q->isize = q->mpos - q->pos - b->hits[0].len;
            } else
                q->isize = 0;
        } else
            q->mchr = q->mpos = -1;
    ...
    if (b->n == 1 && m->n == 1) {
        ...
        if (p->flag & BSW2_FLAG_MATESW) {  // this alignment is found by Smith-Waterman
            ...
            if (!(p->flag & BSW2_FLAG_TANDEM)) {  // pqual is bounded by [b->aux[0].qual,m->aux[0].qual]
                ...
                if (b->aux[0].pqual > m->aux[0].qual)
                    b->aux[0].pqual = m->aux[0].qual;
                if (b->aux[0].pqual < b->aux[0].qual)
                    b->aux[0].pqual = b->aux[0].qual;
            }
        }
    }
}
    ...
    for (i = 0; b && i < b->n; ++i) {
        ...
        if (!is_pe)
            ...
            ksprintf(
                &str,
                "\t%s\t%d\t%d\t",
                q->mchr == q->chr ? "=" : (q->mchr < 0 ? "*" : bns->anns[q->mchr].name),
                q->mpos + 1,
                q->isize);
        ...
        if (opt->hard_clip && q->cigar) {
            if ((q->cigar[0] & 0xf) == 4)
                beg += q->cigar[0] >> 4;
            if ((q->cigar[q->n_cigar - 1] & 0xf) == 4)
                end -= q->cigar[q->n_cigar - 1] >> 4;
        }
        for (j = beg; j < end; ++j) {
            if (p->flag & 0x10)
                kputc(nt_comp_table[(int)ks->seq[ks->l - 1 - j]], &str);
            else
                kputc(ks->seq[j], &str);
        }
        ...
        if (q->nn)
            ksprintf(&str, "\tXN:i:%d", q->nn);
        if (p->l)
            ksprintf(&str, "\tXI:i:%d", p->l - p->k + 1);
        if (p->flag & BSW2_FLAG_MATESW)
            type |= 1;
        if (p->flag & BSW2_FLAG_TANDEM)
            type |= 2;
        if (type)
            ksprintf(&str, "\tXT:i:%d", type);
        if (opt->cpy_cmt && ks->comment) {
            int l = strlen(ks->comment);
            if (l >= 6 && ks->comment[2] == ':' && ks->comment[4] == ':') {
                kputc('\t', &str);
                kputs(ks->comment, &str);
            }
        }
        kputc('\n', &str);
    }
...
static void update_opt(bsw2opt_t* dst, const bsw2opt_t* src, int qlen) {
    ...
    int i, k;
    *dst = *src;
    if (dst->t < ll * dst->coef)
        dst->t = (int)(ll * dst->coef + .499);
...
static void bsw2_aln_core(
    ...
    for (x = 0; x < _seq->n; ++x) {
        ...
        for (i = k = 0; i < l; ++i) {
            ...
            if (c >= 4) {
                c = (int)(drand48() * 4);
                ++k;
            }  // FIXME: ambiguous bases are not properly handled
            seq[0][i] = c;
            seq[1][l - 1 - i] = 3 - c;
            rseq[0][l - 1 - i] = 3 - c;
            rseq[1][i] = c;
        ...
        for (k = 0; k < b[0]->n; ++k)
            if (b[0]->hits[k].n_seeds < opt.t_seeds)
                break;
        if (k < b[0]->n) {
    ...
    if (is_pe)
        bsw2_pair(&opt, bns->l_pac, pac, _seq->n, _seq->seq, buf);
    for (x = 0; x < _seq->n; ++x) {
        bsw2seq1_t* p = _seq->seq + x;
        uint8_t* seq[2];
        int i;
