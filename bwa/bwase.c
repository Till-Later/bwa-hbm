...
bwtint_t bwa_sa2pos(const bntseq_t* bns, const bwt_t* bwt, bwtint_t sapos, int ref_len, int* strand) {
    ...
    *strand = 0;                 // initialise strand to 0 otherwise we could return without setting it
    ...
    if (pos_f < bns->l_pac && bns->l_pac < pos_f + ref_len)
        return (bwtint_t)-1;
    pos_f =
        bns_depos(bns, pos_f, &is_rev);  // position on the forward strand; this may be the first base or the last base
    *strand = !is_rev;
    if (is_rev)
...
bwa_cigar_t* bwa_refine_gapped_core(
    bwtint_t l_pac,
    ...
    assert(*n_cigar > 0);
    if ((cigar32[*n_cigar - 1] & 0xf) == 1)
        cigar32[*n_cigar - 1] = (cigar32[*n_cigar - 1] >> 4 << 4) | 3;  // change endding ins to soft clipping
    if ((cigar32[0] & 0xf) == 1)
        cigar32[0] = (cigar32[0] >> 4 << 4) | 3;  // change beginning ins to soft clipping
    if ((cigar32[*n_cigar - 1] & 0xf) == 2)
        --*n_cigar;                 // delete endding del
...
void bwa_refine_gapped(const bntseq_t* bns, int n_seqs, bwa_seq_t* seqs, ubyte_t* _pacseq) {
    ...
    for (i = 0; i != n_seqs; ++i) {
        ...
        for (j = k = 0; j < s->n_multi; ++j) {
            ...
            if (q->gap) {  // gapped alignment
                q->cigar = bwa_refine_gapped_core(
                    bns->l_pac, pacseq, s->len, q->strand ? s->rseq : s->seq, q->ref_shift, &q->pos, &n_cigar);
                q->n_cigar = n_cigar;
                if (q->cigar)
                    s->multi[k++] = *q;
            } else
        ...
        if (s->type == BWA_TYPE_NO_MATCH || s->type == BWA_TYPE_MATESW || s->n_gapo == 0)
            continue;
        s->cigar = bwa_refine_gapped_core(
            bns->l_pac, pacseq, s->len, s->strand ? s->rseq : s->seq, s->ref_shift, &s->pos, &s->n_cigar);
        if (s->cigar == 0)
            s->type = BWA_TYPE_NO_MATCH;
    }
    // generate MD tag
    ...
    for (i = 0; i != n_seqs; ++i) {
        ...
        if (s->type != BWA_TYPE_NO_MATCH) {
            int nm;
            s->md = bwa_cal_md1(
                s->n_cigar, s->cigar, s->len, s->pos, s->strand ? s->rseq : s->seq, bns->l_pac, pacseq, str, &nm);
            s->nm = nm;
        }
    }
    free(str->s);
