...
if { $impl_flow == "CLOUD_BASE" } {
  ...
  set opt_design_directive [get_property STEPS.OPT_DESIGN.ARGS.DIRECTIVE [get_runs impl_1]]
...
if { [info exists ::env(OPT_DESIGN_DIRECTIVE)] == 1 } {
  set opt_design_directive $env(OPT_DESIGN_DIRECTIVE);
}

if { [info exists ::env(PLACE_DIRECTIVE)] == 1 } {
  set place_directive $env(PLACE_DIRECTIVE);
}

if { [info exists ::env(PHYS_OPT_DIRECTIVE)] == 1 } {
  set phys_opt_directive $env(PHYS_OPT_DIRECTIVE);
}

if { [info exists ::env(ROUTE_DIRECTIVE)] == 1 } {
  set route_directive $env(ROUTE_DIRECTIVE);
}

if { [info exists ::env(OPT_ROUTE_DIRECTIVE)] == 1 } {
  set opt_route_directive $env(OPT_ROUTE_DIRECTIVE);
}
...
if { $cloud_flow == "TRUE" } {
  ...
  set step      opt_design
  set directive $opt_design_directive
}
...
if { ($impl_step == "OPT_ROUTE") } {
  puts [format "%-*s%-*s%-*s%-*s"  $widthCol1 "" $widthCol2 "" $widthCol3 "reload opt_routed_design DCP" $widthCol4 "[clock format [clock seconds] -format {%T %a %b %d %Y}]"]
  open_checkpoint $dcp_dir/${step}.dcp  >> $logfile
}

if { ($impl_step == "ROUTE") || ($impl_step == "OPT_ROUTE") || ($impl_step == "ALL") } {
  set logfile   $logs_dir/${step}.log
  set command   "phys_opt_design  -directive $directive"
