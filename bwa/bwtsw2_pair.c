...
bsw2pestat_t bsw2_stat(int n, bwtsw2_t** buf, kstring_t* msg, int max_ins) {
    ...
    for (i = k = 0; i < n; i += 2) {
        bsw2hit_t* t[2];
        int l;
        if (buf[i] == 0 || buf[i]->n != 1 || buf[i + 1]->n != 1)
            continue;  // more than 1 hits
        t[0] = &buf[i]->hits[0];
        t[1] = &buf[i + 1]->hits[0];
        if (t[0]->G2 > 0.8 * t[0]->G)
            continue;  // the best hit is not good enough
        if (t[1]->G2 > 0.8 * t[1]->G)
            continue;  // the best hit is not good enough
        l = t[0]->k > t[1]->k ? t[0]->k - t[1]->k + t[1]->len : t[1]->k - t[0]->k + t[0]->len;
        if (l >= max_ins)
            continue;  // skip pairs with excessively large insert
    ...
    if (r.low > r.high) {
        ksprintf(
            msg,
            "[%s] fail to infer the insert size distribution: upper bound is smaller than max read length.\n",
            __func__);
        free(isize);
        r.failed = 1;
        return r;
...
void bsw2_pair1(
    ...
    a->n_seeds = 1;
    a->flag |=
        BSW2_FLAG_MATESW;  // before calling this routine, *a has been cleared with memset(0); the flag is set with 1<<6/7
    if (h->is_rev == 0) {
    ...
    if (a->is_rev)
        i = a->beg, a->beg = l_mseq - a->end, a->end = l_mseq - i;
    free(seq);
}
...
void bsw2_pair(const bsw2opt_t* opt, int64_t l_pac, const uint8_t* pac, int n, bsw2seq1_t* seq, bwtsw2_t** hits) {
    ...
    for (i = 0; i < n; i += 2) {
        ...
        if (hits[i] == 0 || hits[i + 1] == 0)
            continue;  // one end has excessive N
        if (hits[i]->n != 1 && hits[i + 1]->n != 1)
            continue;  // no end has exactly one hit
        if (hits[i]->n > 1 || hits[i + 1]->n > 1)
            continue;  // one read has more than one hit
        if (!opt->skip_sw) {
            if (hits[i + 0]->n == 1)
                bsw2_pair1(opt, l_pac, pac, &pes, &hits[i + 0]->hits[0], seq[i + 1].l, seq[i + 1].seq, &a[1], g_mat);
            if (hits[i + 1]->n == 1)
                bsw2_pair1(opt, l_pac, pac, &pes, &hits[i + 1]->hits[0], seq[i + 0].l, seq[i + 0].seq, &a[0], g_mat);
        ...
        if (hits[i]->n + hits[i + 1]->n == 1) {  // one end mapped; the other not;
            ...
            int which;
            if (hits[i]->n == 1)
                p[0] = hits[i], p[1] = hits[i + 1], which = 1;
            else
                p[0] = hits[i + 1], p[1] = hits[i], which = 0;
            if (a[which].G == 0)
                continue;
            ...
            if (hits[i]->hits[0].k == a[0].k
                && hits[i + 1]->hits[0].k == a[1].k) {  // properly paired and no ends need to be moved
                for (j = 0; j < 2; ++j)
                ...
                if (a[0].G && a[1].G) {                    // now we have two "proper pairs"
                    ...
                    diff = fabs((double)(G[0] - G[1])) / (opt->a + opt->b)
                        / ((hits[i]->hits[0].len + a[1].len + hits[i + 1]->hits[0].len + a[0].len) / 2.);
                    if (diff > 0.05)
                        a[G[0] > G[1] ? 0 : 1].G = 0;
                }
                if (a[0].G == 0 || a[1].G == 0) {  // one proper pair only
                    ...
                    double dev, diff;
                    if (a[0].G)
                        p[0] = &hits[i + 1]->hits[0], p[1] = &hits[i]->hits[0], which = 0;
                    else
                        p[0] = &hits[i]->hits[0], p[1] = &hits[i + 1]->hits[0], which = 1;
