...
static const char* err_msg[MAX_ERR + 1] = {"successful merges",
                                           "low-scoring pairs",
...
int bwa_pemerge(const pem_opt_t* opt, bseq1_t x[2]) {
    ...
    s[0] = malloc(x[0].l_seq);
    q[0] = malloc(x[0].l_seq);
    s[1] = malloc(x[1].l_seq);
    q[1] = malloc(x[1].l_seq);
    ...
    ++r.qe;
    ++r.te;  // change to the half-close-half-open coordinates
    ...
    if (r.tb < r.qb) {
        ret = -2;
        goto pem_ret;
    }  // no enough space for the left end
    if (x[0].l_seq - r.te > x[1].l_seq - r.qe) {
        ret = -3;
        goto pem_ret;
    }  // no enough space for the right end
    if ((double)r.score2 / r.score >= MAX_SCORE_RATIO) {
        ret = -4;
        goto pem_ret;
    }  // the second best score is too large
    if (r.qe - r.qb != r.te - r.tb) {
        ret = -5;
        goto pem_ret;
    ...
    {  // test tandem match; O(n^2)
        ...
        for (l = 1; l < min_l; ++l) {
            ...
            if (m > max_m)
                max_m2 = max_m, max_m = m, max_l2 = max_l, max_l = l;
            else if (m > max_m2)
                max_m2 = m, max_l2 = l;
        }
        if (max_m < opt->T || max_l != x[0].l_seq - (r.tb - r.qb)) {
            ret = -6;
            goto pem_ret;
        }
        if (max_l2 < max_l && max_m2 >= opt->T
            && (double)(max_m2 + (max_l - max_l2) * opt->a) / max_m >= MAX_SCORE_RATIO) {
            ret = -7;
            goto pem_ret;
        }
        if (max_l2 > max_l && (double)max_m2 / max_m >= MAX_SCORE_RATIO) {
            ret = -7;
            goto pem_ret;
        }
    }
    ...
    seq = malloc(l_seq + 1);
    qual = malloc(l_seq + 1);
    memcpy(seq, s[0], x[0].l_seq);
    memcpy(seq + x[0].l_seq, &s[1][l], x[1].l_seq - l);
    memcpy(qual, q[0], x[0].l_seq);
    memcpy(qual + x[0].l_seq, &q[1][l], x[1].l_seq - l);
    ...
    for (i = 0; i < l_seq; ++i)
        seq[i] = "ACGTN"[(int)seq[i]], qual[i] += 33;
...
int main_pemerge(int argc, char* argv[]) {
    ...
    while ((c = getopt(argc, argv, "muQ:t:T:")) >= 0) {
        if (c == 'm')
            ...
            opt->q_thres = atoi(optarg);
        else if (c == 't')
            opt->n_threads = atoi(optarg);
        else if (c == 'T')
            min_ovlp = atoi(optarg);
        else
            return 1;
    }
    if (flag == 0)
