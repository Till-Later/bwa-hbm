...
if { ([info exists ::env(ADD_PBLOCK)] == 1) } {
  if {  $::env(ADD_PBLOCK) eq "BSP_HBM_HMEM"  } {
    puts "                        Adding pblock (BSP_HBM_HMEM)\n"    
    add_files -fileset constrs_1 -norecurse $top_xdc_dir/pblocks/pblock_bsp_hbm_hmem.xdc
    set_property used_in_synthesis true [get_files  $top_xdc_dir/pblocks/pblock_bsp_hbm_hmem.xdc]
    set_property used_in_implementation true [get_files  $top_xdc_dir/pblocks/pblock_bsp_hbm_hmem.xdc]
  } elseif {  $::env(ADD_PBLOCK_BOARD_SUPPORT) eq "SMEM_SLR2_BWT_SLR0"  } {
    puts "                        Adding pblock_board_support (SMEM_SLR2_BWT_SLR0)\n"    
    add_files -fileset constrs_1 -norecurse $top_xdc_dir/pblocks/pblock_smem_slr2_bwt_slr0.xdc
    set_property used_in_synthesis true [get_files  $top_xdc_dir/pblocks/pblock_smem_slr2_bwt_slr0.xdc]
    set_property used_in_implementation true [get_files  $top_xdc_dir/pblocks/pblock_smem_slr2_bwt_slr0.xdc]
  }
} else {
  puts "                        Board support remains unconstrained.\n"
}
